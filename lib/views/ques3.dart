...
class _QuestionnaireAppState extends State<QuestionnaireApp> with TickerProviderStateMixin {
  ...
  double _inactiveTabWidth = 10.0;
  double _activeTabWidth = 150.0;
  ...
  Widget buildSliderQuestion(Question question) {
    return ListTile(
      title: Text(question.text),
      subtitle: Slider(
        value: _selectedAnswers?[question.text]?.toDouble() ?? question.minSliderValue ?? 0.0,
        min: question.minSliderValue ?? 0.0,
        max: question.maxSliderValue ?? 100.0,
        onChanged: (value) {
          setState(() {
            _selectedAnswers?[question.text] = value.toInt();
          });
        },
      ),
    );
                  ...
                  child: AppBar(
                    ...
                    bottom: TabBar(
                      ...
                      tabs: sections.map((section) {
                        return Container(
                          width: _currentTabIndex == sections.indexOf(section) ? _activeTabWidth : _inactiveTabWidth,
                          child: Tab(
